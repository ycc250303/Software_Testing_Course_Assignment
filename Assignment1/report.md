...
以下以 `POST /users` 为示例展示三轮对话流程；8 个接口的完整对话记录详见附录 A。
...
| ------------ | -------------------------------------------------------------------------- | --------------------- |
| TC01         | Register with valid username/password/email/verificationCode               | HTTP 200, code = 0    |
...
| TC07         | Register with invalid email format                                         | HTTP 400, code = 1001 |
...
| TC10         | Register with valid username/email but expired or invalid verificationCode | HTTP 400, code = 2003 |
...
**input（接口描述）**
与 v1 相同，保持同一 `POST /users` 接口需求输入。
...
| Test Case ID | Scenario                                                                      | Expected Result                       |
| ------------ | ----------------------------------------------------------------------------- | ------------------------------------- |
| TC01         | valid username/password/email and valid non-expired verificationCode          | HTTP 200, code=0                      |
| TC02         | username missing                                                              | HTTP 400, code=1001                   |
| TC03         | username length=2                                                             | HTTP 400, code=1001                   |
| TC04         | username length=51                                                            | HTTP 400, code=1001                   |
| TC05         | password length=5                                                             | HTTP 400, code=1001                   |
| TC06         | password length=21                                                            | HTTP 400, code=1001                   |
| TC07         | email invalid format                                                          | HTTP 400, code=1001                   |
| TC08         | email length=101                                                              | HTTP 400, code=1001                   |
| TC09         | verificationCode=5 digits or contains non-digit                               | HTTP 400, code=1001                   |
| TC10         | business errors: existing username/email, invalid or expired verificationCode | corresponding business error returned |
...
**input（接口描述）**
与 v1/v2 相同，保持同一 `POST /users` 接口需求输入。
...
下表汇总了6.1 POST /users的关键数据与分类结果，便于后续对比与分析。
...
下表汇总了6.2 GET /users/ {id}的关键数据与分类结果，便于后续对比与分析。
...
下表汇总了6.3 PUT /users/password的关键数据与分类结果，便于后续对比与分析。
...
下表汇总了6.4 POST /educations的关键数据与分类结果，便于后续对比与分析。
...
下表汇总了6.5 PUT /educations的关键数据与分类结果，便于后续对比与分析。
...
下表汇总了6.6 GET /jobs的关键数据与分类结果，便于后续对比与分析。
...
后续可从方向继续改进：优化提示词，减少对人工调整的依赖；加强对跨字段约束等复杂场景的生成能力；将测试用例生成与自动执行结合，减少人工参与；引入更多评估指标，提高结果分析的全面性。
